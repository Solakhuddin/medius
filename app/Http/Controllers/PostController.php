...
class PostController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->validate([
            ...
            'category' => 'required|string|max:50', // Validasi untuk nama kategori
        ...
        $category = Category::firstOrCreate(
            ...
            ['name' => $categoryName] // Data untuk membuat jika tidak ditemukan
        );  
        $data['category_id'] = $category->id; // Simpan ID kategori yang baru dibuat atau ditemukan
    ...
    {
        // Eager load relasi user dan comments (beserta user dari setiap comment)
        ...
        return view('post.show', [
            'post' => $post,
            'isBookmarked' => $isBookmarked, // Kirim status bookmark ke view
        ]);
        // return view('post.show', [
        //     'post' => $post,
        // ]);
    ...
    public function update(PostUpdateRequest $request, Post $post)
    {
        ...
        // if it does, clear the previous media and add the new one
        // if not, it will keep the previous media
    ...
    public function destroy(Post $post)
    {
        if (auth()->user()->id !== $post->user_id) {
            abort(403, 'You are not authorized to delete this post.');
            // Alternatively, you can redirect with an error message
